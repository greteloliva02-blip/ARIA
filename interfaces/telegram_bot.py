...
class TelegramBot(BaseInterface):
    """Telegram bot interface for ARIA (polling only)."""
    ...
    async def _cmd_memory(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        facts = await asyncio.to_thread(self.agent.memory.get_facts, uid)
        if not facts:
    ...
    async def start(self):
        """Start Telegram bot in pure async polling mode."""
        logger.info("Starting Telegram bot (pure polling mode)...")
    ...
    async def stop(self):
        ...
        try:
            ...
            logger.warning("Application stop failed: %s", e)
        try:
            await self.app.shutdown()
        except Exception as e:
            logger.warning("Application shutdown failed: %s", e)
